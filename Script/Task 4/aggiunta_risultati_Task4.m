%% Aggiunta Task 4 a tabella "risultati"
j = 1;
temp = table(NaN(height(risultati),1));

for i = 1:height(risultati)
    if (table2array(risultati(i,4)) == 1)
        temp(i,1) = responseTask4(j,1);
        j = j+1;   
    end
end
temp.Properties.VariableNames = "Task 4";

% Eliminazione eventuali duplicati
if ismember('Task 4',risultati.Properties.VariableNames)
    risultati = removevars(risultati, 'Task 4');
end
